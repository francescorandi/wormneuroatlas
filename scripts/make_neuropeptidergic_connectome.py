...
# EC50.
watlas.supplement_peptide_gpcr()

# Get the list of all peptide and GPCR names contained in the deorphanization
# screen.
peptide_names = np.array(watlas.get_peptide_names(trim_isoforms=True))
gpcr_names = np.array(watlas.get_gpcr_names(trim_isoforms=True))
gpcr_seq_ids = np.array(watlas.get_gpcr_seq_id_from_name(gpcr_names))
...
pep_esconn2 = watlas.get_peptidergic_connectome()
...
for j in np.arange(watlas.neuron_n):
    ...
    gpcr_binding_seq_ids = \
          watlas.get_gpcrs_binding_to(pep_expressed,return_seq_ids=True)
...
print("Testing if the method built into the NeuroAtlas class returns the same",
      "values as the original script.")
print(np.allclose(pep_esconn2,pep_esconn))
